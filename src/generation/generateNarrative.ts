...
export function generateMarkdown(
  ...
  profileDetails.forEach(profileDetail => {
    try {
      logger.info('');
      logger.info(`** generating markdown for profile of ${profileDetail.getParentProfileUrl()}`);
      ...
      if (codeFilter.valueSet) {
        ...
        if (valueSetDetails) {
          markdownContent +=
            ...
            }' target='_blank'>${valueSetDetails.id ?? codeFilter.valueSet}</a></td>\n` +
