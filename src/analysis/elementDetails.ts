...
export class ElementDetail {
  ...
  public determineDisplayDetails(
    ...
    if (settings.elementDetailOverrides.has(overrideKey)) {
      ...
      logger.warn(
        `Nested ${this.isMeasureRequirement() ? 'measure' : 'inherited'} must support element ${
          this.elementId
...
export class ProfileDetail {
  ...
  private resolveExtensionDefinition(elementId: string): fhir4.ElementDefinition | undefined {
    ...
    if (firstChar != "'" && firstChar != '"') {
      logger.error(`bad extension expression in path - no quote containing the URL: ${elementId}`);
      return undefined;
    }
...
export async function prepareElementDetails(
  ...
  profileDetails.forEach(profileDetail => profileDetail.addMustSupportElementsFromParentProfile());
...
async function dataReqsToProfileDetail(dataReqs: DataReqOutputs[], settings: RuntimeSettings) {
  ...
  dataReqs.forEach(dataReq => {
    ...
    if (dataReq.mustSupport && dataReq.mustSupport.length == 0) {
      logger.warn('No MustSupport element found: Skipping creating profile for ' + dataReq.type);
    } else {
      try {
        ...
        dataReq.mustSupport?.forEach((element, elementIndex) => {
          ...
          if (elementDefinition) {
            ...
            dataReq.codeFilter.forEach(oneCodeFilter => {
              if (oneCodeFilter.path === element) {
                if (oneCodeFilter.code || oneCodeFilter.valueSet) {
                  ...
                  logger.warn(`filter on element ${element} has no code or value set, skipping`);
                }
              }
            });
            ...
            profileDetails.addElement(
              ...
              new ElementDetail(elementDefinition.id!, measureList, elementDefinition, filters)
            );

            /// primary code filters for this resource type
            if (
              KeyFilterElementForResourceType.get(profileDetails.getResourceType()) ===
              elementDefinition.id!
            ) {
...
export function getRemoteProfile(
  ...
  if (profileDef) {
    ...
    logger.warn('Profile not found using package loader - check dependencies. Using direct fetch.');
    try {
