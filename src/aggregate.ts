        ...
        if (measureFlg) {
            bundle.entry?.forEach(entry => {
                ...
                if (
                    ...
                    logger.info(
                        `**** Processing data requirements in library with id ${entry.resource!.id}`
                    );
                            ...
                            if (!ResourceTypesWithoutKeyFilterElements.has(item.type)) {
                                if (KeyFilterElementForResourceType.has(item.type)) {
                                    ...
                                    const hasFilterOnElement: boolean = (
                                        ...
                                        (wasAlreadyFound: boolean, foundHere: boolean) =>
                                            wasAlreadyFound || foundHere,
                                        false
                                    );

                                    if (!hasFilterOnElement) {
                                        logger.error(
                                            errMsg.noFilterOnKeyElementMsg(
                                                item.type,
                                                p,
                                                filterElement
                                            )
                                    ...
                                    if (dro.profile === p) {
                                        ...
                                        item.mustSupport?.forEach(ms => {
                                            if (dro.mustSupport?.includes(ms)) {
                                                if (
                                                    !dro._mustSupport[
                                                        dro.mustSupport?.indexOf(ms)
                                                    ].extension.some(
                                                        ext =>
                                                            ext.url ===
                                                                settings.measureExtensionURL &&
                                                            ext.valueString ===
                                                                measureExt.valueString
                                                    )
                                                ) {
                                                    dro._mustSupport[
                                                        dro.mustSupport?.indexOf(ms)
                                                    ].extension.push(measureExt);
                                                }
                                            } else {
                                        ...
                                        new_cf.forEach(it_cf => {
                                            if (it_cf.valueSet) {
                                                ...
                                                if (matchingFilters.length === 0) {
                                                    ...
                                                    logger.error(
                                                        `multiple matching filters for value set ${it_cf.valueSet}`
                                                    );
                                                }
                                            }
                                            ...
                                            if (it_cf.code) {
                                                const matchingFilters = dro.codeFilter.filter(
                                                    oneCodeFilter =>
                                                        oneCodeFilter.code?.at(0)?.system ===
                                                            it_cf.code?.at(0)!.system &&
                                                        oneCodeFilter.code?.at(0)?.code ===
                                                            it_cf.code?.at(0)!.code &&
                                                        oneCodeFilter.path === it_cf.path
                                                );
                                                if (matchingFilters.length === 0) {
                                                    // no filter entry for this value set yet, add
                                                    ...
                                                    logger.error(
                                                        `multiple matching filters for code ${it_cf.code.at(
                                                            0
                                                        )}`
                                                    );
                                                }
