...
import { ElementDetail, ProfileDetail } from '../analysis/elementDetails';
import { getMeasureListString } from '../common/util';

export function generateFSH(profileDetails: ProfileDetail[], settings: RuntimeSettings): string {
  let fsh: string = fs.readFileSync('src/fixtures/extension.fsh', { encoding: 'utf8', flag: 'r' });
...
function generateFSHProfileHeader(oneProfile: ProfileDetail, settings: RuntimeSettings): string {
  ...
  fsh += `Description: "This profile defines implementation-time requirements for data used in the calculation of ${
    settings.projectMarkdownNameAndLink ? settings.projectMarkdownNameAndLink : settings.projectName
  } quality measures."\n`;
...
function generateFSHProfileElement(
  ...
  fsh += fshPrefixedShort(elementPath, '(Measure)', oneElement.getElementDefinition().short ?? '');
